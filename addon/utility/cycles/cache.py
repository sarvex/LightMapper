...
def backup_material_copy(slot):
    ...
    dup = material.copy()
    dup.name = f".{material.name}_Original"
...
def backup_material_restore(obj): #??
    if bpy.context.scene.TLM_SceneProperties.tlm_verbose:
        print(f"Restoring material for: {obj.name}")
        ...
        if len(prevMatArray) > 0:
            for idx, slot in enumerate(obj.material_slots): #For each slot, we get the index
                ...
                if (
                    ...
                    and f".{originalMaterial}_Original" in bpy.data.materials
                ):
                    slot.material = bpy.data.materials[f".{originalMaterial}_Original"]
                    slot.material.use_fake_user = False

        else:

            print(f"No previous material for {obj.name}")
...
def backup_material_rename(obj): #??
    if bpy.context.scene.TLM_SceneProperties.tlm_verbose:
        print(f"Renaming material for: {obj.name}")
    ...
    if "TLM_PrevMatArray" in obj:
        ...
        for slot in obj.material_slots:
            ...
            if slot.material is not None and slot.material.name.endswith(
                "_Original"
            ):
                newname = slot.material.name[1:-9]
                if (
                    newname in bpy.data.materials
                    and bpy.context.scene.TLM_SceneProperties.tlm_verbose
                ):
                    print(f"Removing material: {bpy.data.materials[newname].name}")
        ...
        print(f"No Previous material array for: {obj.name}")