...
def bake(plus_pass=0):
        ...
        if obj.type == 'MESH' and obj.name in bpy.context.view_layer.objects:
            ...
            for collection in collections:
                ...
                try:
                    if (
                        collection.name
                        in bpy.context.scene.view_layers[
                            0
                        ].layer_collection.children
                        and bpy.context.scene.view_layers[0]
                        .layer_collection.children[collection.name]
                        .hide_viewport
                    ):
                        hidden = True
                except:
                    print(f"Error: Could not find collection: {collection.name}")
    ...
    for obj in bpy.context.scene.objects:
        ...
        if bpy.context.scene.TLM_SceneProperties.tlm_verbose:
            print(f"Checking visibility status for object and collections: {obj.name}")
        ...
        if (
            ...
            scene.render.bake.use_clear = False
                ...
                #if bpy.context.scene.TLM_SceneProperties.tlm_verbose:
            print(
                f"Baking {str(currentIterNum)}/{str(iterNum)} ({str(round(currentIterNum / iterNum * 100, 2))}%) : {obj.name}"
            )
            ...
            print(
                f"Elapsed time: {str(round(elapsedSeconds, 2))}s | ETA remaining: {str(round(remaining, 2))}s"
            )
            ...
            if scene.TLM_EngineProperties.tlm_lighting_mode == "ao":
            ...
            elif scene.TLM_EngineProperties.tlm_lighting_mode == "combined":
            ...
            if scene.TLM_SceneProperties.tlm_save_preprocess_lightmaps:
                for image in bpy.data.images:
                    if image.name.endswith("_baked"):
                        ...
                        image.filepath_raw = f"{bakemap_path}.hdr"
                        image.file_format = "HDR"
                        if bpy.context.scene.TLM_SceneProperties.tlm_verbose:
                            print(f"Saving to: {image.filepath_raw}")
                        image.save()
